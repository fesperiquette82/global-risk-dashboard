...
export function computeDashboardModel(indicators: Indicator[]): DashboardModel {
  const vix = get(indicators, 'vix') ?? 20;
  ...
  const cpiYoy = get(indicators, 'cpi_yoy') ?? 2;
  const creditSpread = get(indicators, 'credit_spread_hy') ?? 3.5;
  const btc7 = indicators.find((i) => i.id === 'btc')?.change7d ?? 0;
  const geo = get(indicators, 'gdelt_volume') ?? 20;

  // VIX>15 elevated, spread HY>3% normal floor, CPI YoY 2% = Fed target — see docs/SPEC-decision-support.md §5.
  const rates = clamp100((vix - 15) * 2 + Math.max(0, oil - 80));
  const infl = clamp100(Math.max(0, (cpiYoy - 2) * 25));
  ...
  const credit = clamp100(Math.max(0, (creditSpread - 3) * 20));
  const market = clamp100((rates + growth + crypto) / 3);
  const global = clamp100(Math.max(rates, infl, growth, geoS, crypto, commod, credit, market));
